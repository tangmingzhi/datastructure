...
public class BloomFilter<E> implements Serializable {
    ...
    public void add(E element) {
        ...
        for (int x = 0; x < k; x++) {
            ...
            //hash 为存放bitset中index索引
            //true 标记为该索引值为true
         //   System.out.println(Math.abs((int) hash));
    ...
    public boolean contains(E element) {
        ...
        for (int x = 0; x < k; x++) {
            hash = createHash(valString + Integer.toString(x));
            hash = hash % (long) bitSetSize;
            boolean flag = !bitset.get(Math.abs((int) hash));
            System.out.println("hash:"+hash+" flag:"+flag);
            if (flag) {
                return false;
            }
        }
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        BloomFilter bloomFilter = new BloomFilter(8,wordSize,7);
        long time = System.currentTimeMillis();
        for (int i = 0; i < wordSize; i++) {
            bloomFilter.add(keyPre+i);
        }
        System.out.println("添加时间："+(System.currentTimeMillis()-time));
        time=System.currentTimeMillis();
        System.out.println(bloomFilter.contains((wordSize+2)+keyPre));
